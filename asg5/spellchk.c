...
void spellcheck (char *filename, hashset_ref hashset) {
   ...
   for (;;) {
	 ...
	 char *lowerword = malloc(sizeof(char)*(strlen(yytext)+1));
	 for(unsigned int x = 0; x < strlen(yytext); x++) {
	    lowerword[x] = tolower(yytext[x]);       
         }
         if(!has_hashset(hashset, lowerword)) {
...
void load_dictionary (char *dictionary_name, struct hashset *hashset) {
   ...
   char buffer[1024];
   FILE *p = open_infile(dictionary_name);
   if(p == NULL) {
     return;
   }
   for(;;) {
     char *newstring = fgets(buffer, sizeof(buffer), p);
     ...
     put_hashset(hashset, strdup(buffer));
   }
